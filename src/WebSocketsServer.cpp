...
void WebSocketsServer::begin(void) {
    ...
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        ...
        client->cWsRXsize = 0;
		client->isSubscribedSerial = false;
...
bool WebSocketsServer::broadcastSerialOverWiFi(uint8_t * payload, size_t length, bool headerToPayload) {
    WSclient_t * client;
    bool ret = true;
    if(length == 0) {
        length = strlen((const char *)payload);
    ...
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
        if(clientIsConnected(client) && clientIsSubscribed(client)) {
            if(!sendFrame(client, WSop_text, payload, length, true, headerToPayload)) {
                ret = false;
            }
        }
...
bool WebSocketsServer::broadcastSerialOverWiFi(const uint8_t * payload, size_t length) {
    return broadcastSerialOverWiFi((uint8_t *)payload, length);
}

bool WebSocketsServer::broadcastSerialOverWiFi(char * payload, size_t length, bool headerToPayload) {
    return broadcastSerialOverWiFi((uint8_t *)payload, length, headerToPayload);
}

bool WebSocketsServer::broadcastSerialOverWiFi(const char * payload, size_t length) {
    return broadcastSerialOverWiFi((uint8_t *)payload, length);
}

bool WebSocketsServer::broadcastSerialOverWiFi(String & payload) {
    return broadcastSerialOverWiFi((uint8_t *)payload.c_str(), payload.length());
}
...
void WebSocketsServer::subscribe(uint8_t num) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return;
    }
    WSclient_t * client = &_clients[num];
    if(!clientIsSubscribed(client)) {
        client->isSubscribedSerial = true;
    }
}

void WebSocketsServer::unsubscribe(uint8_t num) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return;
    }
    WSclient_t * client = &_clients[num];
...
bool WebSocketsServer::clientIsSubscribed(WSclient_t * client) {
    if(client->isSubscribedSerial) {
        return true;
    }
    return false;
}
