...
class WebSocketsServer : protected WebSockets {
  public:
	...
	void subscribe(uint8_t num);	//This will subscribe the client to receive the Serial data over wifi.
	void unsubscribe(uint8_t num);  //This will unsubscribe the client from receiving Serial data over wifi.
  ...
  protected:
	...
	bool clientIsSubscribed(WSclient_t * client);
